...
class SharePointAPI:
    ...
    def authenticate(self):
        ...
        try:
            authcookie = Office365(
                self.url, username=self.user, password=self.password
            ).GetCookies()
            site = Site(self.site, version=Version.v365, authcookie=authcookie)
    ...
    def get_sp_list(self, site, list):
        """
    ...
    def create_list_items(self, list, items):
        """
        ...
        if len(items) > 0:
            try:
                list.UpdateListItems(data=items, kind="New")
            except:
                # We should log the specific type of error occurred.
                print("Failed to upload new list items: {}".format(sys.exc_info()[1]))

    def download_list_items(
        self, list, view_name=None, fields=None, query=None, row_limit=0
    ):
        """
        ...
        """
        try:
            list_items = list.GetListItems(
                view_name=view_name, fields=fields, query=query, row_limit=row_limit
            )
        except:
            # We should log the specific type of error occurred.
            print("Failed to download list items {}".format(sys.exc_info()[1]))
            raise SystemExit(
                "Failed to download list items {}".format(sys.exc_info()[1])
            )
        return list_items

    def update_list_items(self, list, items):
        """
        ...
        if len(items) > 0:
            try:
                list.UpdateListItems(data=items, kind="Update")
            except:
                # We should log the specific type of error occurred.
                print(
                    "Failed to update the SharePoint list itemst: {}".format(
                        sys.exc_info()[1]
                    )
                )

    def detele_list_items(self, list, items):
        """
        Takes a SharePoint List instance and a list of ID values.
        """
        if len(items) > 0:
            try:
                list.UpdateListItems(data=items, kind="Delete")
            except:
                # We should log the specific type of error occurred.
