...
class BexioAPI:
    ...
    def get_countries(self):
        ...
        response = requests.request('GET', self.url + self.endpoint, headers=self.headers)
        r = response.text
        res = json.loads(r)    
        return(res)
    ...
    def get_contact_relation_by_contact_sub_id(self, contact_sub_id):
        ...
        response = requests.request('POST', self.url + self.endpoint, data=None, headers=self.headers, params=self.params)
        r = response.text
        res = json.loads(r)
        return(res)    
    ...
    def search_contact_relation(self, payload):
        ...
        '''
        self.endpoint = '/2.0/contact_relation/search'
        response = requests.request('POST', self.url + self.endpoint, json=payload, headers=self.headers)
        r = response.text
        res = json.loads(r)    
        return(res)
    ...
    def get_contacts(self):
        ...
        response = requests.request('GET', self.url + self.endpoint, headers=self.headers, params=self.params)
        r = response.text
        res = json.loads(r)  
        return(res)
    ...
    def search_contact(self, payload):
        ...
        self.params.update({'limit': '5000'})
        response = requests.request('POST', self.url + self.endpoint, json=payload, headers=self.headers, params=self.params)
        r = response.text
        res = json.loads(r)    
        return(res)
