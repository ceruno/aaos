...
def update(args):
    ...
    for config in list(bexio_config):
        ...
        payload_open_quotes = [
            {"field": "kb_item_status_id", "value": "2", "criteria": "="}
        ]

        payload_win_quotes_dyn_mth = [
            {"field": "kb_item_status_id", "value": "3", "criteria": "="},
            {"field": "updated_at", "value": test, "criteria": ">"},
        ]
        ...
        payload_open_projects = [
            {"field": "pr_state_id", "value": "3", "criteria": "<"}
        ]

        payload_contact_group_kunden = [
            {"field": "contact_group_ids", "value": "2", "criteria": "="},
            {"field": "contact_type_id", "value": "1", "criteria": "="},
        ]
        ...
        for item in bexio_customers:
            ...
            payload_invoice = [
                {"field": "contact_id", "value": item["id"], "criteria": "="},
                {"field": "is_valid_from", "value": zeitab, "criteria": ">"},
            ]
        ...
        for item in bexio_open_projects:
            ...
            if "Presales" in item["name"]:
                ...
                payload = [
                    {"field": "pr_project_id", "value": item["id"], "criteria": "="}
                ]
        ...
        for sp_config in list(sharepoint_config):
            ...
            sp_session = SharePointAPI(
                sp_config["sharepoint_url"],
                sp_config["sharepoint_site"],
                sp_config["user"],
                password,
            )
            ...
            def update_sharepoint_lists(list, bexio_list, field_1="ID", field_2="AN"):
                ...
                current_sharepoint_list = sp_session.download_list_items(
                    sp_list, fields=[field_1, field_2]
                )
                ...
                new_bexio = (
                    []
                )  # Liste mit neuen Einträgen im Bexio (nur mit field_2 drinnen)
                new_bexio_entries = (
                    []
                )  # Liste mit neuen Einträgen im Bexio mit allen gewünschten Details
                existing_bexio = (
                    []
                )  # Liste mit Sharepoint Elementen, welche auch noch in der Bexio Liste drinnen sind
                existing_bexio_entries = (
                    []
                )  # Um existierende Einträge zu aktualisieren benötigen wir die dazugehörigen Sharepoint IDs !!
                for s in bexio_list:
                    if (
                        s[field_1] in sharepoint_dic
                    ):  # Diesen Eintrag gibt es schon auf dem Sharepoint
                        val = sharepoint_dic[
                            s[field_1]
                        ]  # val enthält nun die Sharepoint ID
                        s[field_2] = val
                        existing_bexio_entries.append(
                            s
                        )  # BEXIO_LIST mit der ID vom Sharepoint ergänzen
